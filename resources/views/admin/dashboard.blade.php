    ...
    <div class="col-12 mb-3">
        <div class="card bg-primary text-white shadow">
            <div class="card-body d-flex justify-content-between align-items-center py-3">
                <div>
                    <h4 class="mb-0"><i class="mdi mdi-hospital-box-outline"></i> {{ $poli->nama }}</h4>
                    <small>Dashboard Admin Antrian</small>
                </div>
...
function updateUI(data) {
    ...
    document.getElementById('listWaiting').innerHTML = waiting.map(a => `
        <tr>
            ...
            <td>${a.nama}<br><small class="text-muted">${a.poli?.nama ?? ''}</small></td>
            <td class="text-end pe-3">
                <button class="btn btn-sm btn-outline-danger" onclick="lewati(${a.id})">
                    <i class="mdi mdi-account-arrow-right"></i> Lewati
                </button>
            </td>
        </tr>
